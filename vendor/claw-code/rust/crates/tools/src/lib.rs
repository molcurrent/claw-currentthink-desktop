...
mod tests {
    ...
    #[test]
    fn allowed_tools_normalizes_web_search_and_fetch_cases() {
        let registry = GlobalToolRegistry::builtin();

        for raw in ["web_search", "web-search", "websearch", "WebSearch"] {
            let allowed = registry
                .normalize_allowed_tools(&[raw.to_string()])
                .expect("web search input should be normalized cleanly")
                .expect("allowed tools should be some");
            assert!(allowed.contains("WebSearch"), "failed to normalize {raw}");
        }

        for raw in ["web_fetch", "web-fetch", "webfetch", "WebFetch"] {
            let allowed = registry
                .normalize_allowed_tools(&[raw.to_string()])
                .expect("web fetch input should be normalized cleanly")
                .expect("allowed tools should be some");
            assert!(allowed.contains("WebFetch"), "failed to normalize {raw}");
        }
    }
