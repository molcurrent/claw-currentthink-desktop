...
fn strip_unsupported_beta_body_fields(body: &mut Value) {
    if let Some(object) = body.as_object_mut() {
        ...
        // Strip the routing prefix for deepseek models under Anthropic compatibility
        if let Some(model) = object.get_mut("model") {
            if let Some(model_str) = model.as_str() {
                if model_str.starts_with("anthropic/deepseek-") {
                    *model = Value::String(model_str["anthropic/".len()..].to_string());
                }
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn strip_unsupported_beta_body_fields_strips_deepseek_prefix() {
        let mut body = serde_json::json!({
