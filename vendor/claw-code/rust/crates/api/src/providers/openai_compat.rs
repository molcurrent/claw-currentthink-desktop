...
fn wire_model_for_base_url<'a>(
    ...
    let lowered_model_id = model_id.to_ascii_lowercase();

    let trimmed_base_url = base_url.trim_end_matches('/');
    let default_openai = DEFAULT_OPENAI_BASE_URL.trim_end_matches('/');
    let is_nvidia = trimmed_base_url.contains("nvidia") || trimmed_base_url.contains("integrate.api.nvidia.com");

    // Single-provider model preset keywords that must have their prefixes stripped
    // when calling their official, dedicated endpoints.
    let is_preset_single_provider = lowered_model_id.contains("deepseek")
        || lowered_model_id.contains("glm")
        || lowered_model_id.contains("kimi")
        || lowered_model_id.contains("minimax")
        || lowered_model_id.contains("mimo")
        || lowered_model_id.contains("qwen");

    if lowered_prefix == "openai" {
        if is_preset_single_provider {
            // Preset models routed via openai/ to single-provider endpoints must be stripped.
            return Cow::Borrowed(model_id);
        }
        if trimmed_base_url != default_openai {
            // General gateway/aggregator endpoints (like OpenRouter or custom proxies)
            // commonly use slash-containing slugs for generic models. Preserve the prefix.
            return Cow::Borrowed(model);
        }
        return Cow::Borrowed(model_id);
    }

    // List of known routing prefixes that we might strip for single-provider endpoints
    let known_prefixes = [
        "xai",
        "grok",
        "qwen",
    ...
    if known_prefixes.contains(&lowered_prefix.as_str()) {
        // Presets on multi-provider registries like NVIDIA NIM or OpenRouter must preserve prefixes.
        if is_nvidia || trimmed_base_url.contains("openrouter.ai") {
            return Cow::Borrowed(model);
        }
        // Direct single-provider endpoints expect stripped model IDs.
        return Cow::Borrowed(model_id);
    }
...
pub fn read_base_url(config: OpenAiCompatConfig) -> String {
    let mut url = std::env::var(config.base_url_env).unwrap_or_else(|_| config.default_base_url.to_string());
    let trimmed = url.trim_end_matches('/');
    if trimmed.to_ascii_lowercase().ends_with("/anthropic") {
        url = trimmed[..trimmed.len() - "/anthropic".len()].to_string();
    }
    url
}
...
mod tests {
    ...
    #[test]
    fn test_wire_model_for_base_url_preset_compatibility() {
        use super::wire_model_for_base_url;
        let config = OpenAiCompatConfig::openai();

        // 1. Z.AI (GLM) single-provider endpoint strips openai/ prefix
        assert_eq!(
            wire_model_for_base_url("openai/glm-5.1", config.clone(), "https://open.bigmodel.cn/api/paas/v4"),
            "glm-5.1"
        );

        // 2. Kimi (Moonshot) single-provider endpoint strips openai/ prefix
        assert_eq!(
            wire_model_for_base_url("openai/kimi-k2.6", config.clone(), "https://api.moonshot.cn/v1"),
            "kimi-k2.6"
        );

        // 3. MiniMax single-provider endpoint strips openai/ prefix
        assert_eq!(
            wire_model_for_base_url("openai/MiniMax-M2.7", config.clone(), "https://api.minimax.io/v1"),
            "MiniMax-M2.7"
        );

        // 4. Xiaomi MiMo single-provider endpoint strips openai/ prefix
        assert_eq!(
            wire_model_for_base_url("openai/mimo-v2.5", config.clone(), "https://api.xiaomimimo.com/v1"),
            "mimo-v2.5"
        );

        // 5. DeepSeek single-provider endpoint strips openai/ prefix
        assert_eq!(
            wire_model_for_base_url("openai/deepseek-v4-pro", config.clone(), "https://api.deepseek.com/v1"),
            "deepseek-v4-pro"
        );

        // 6. OpenRouter gateway preserves openai/ prefix
        assert_eq!(
            wire_model_for_base_url("openai/gpt-oss-120b", config.clone(), "https://openrouter.ai/api/v1"),
            "openai/gpt-oss-120b"
        );

        // 7. NVIDIA NIM gateway preserves namespaced presets
        assert_eq!(
            wire_model_for_base_url("qwen/qwen3-5-122b-a10b", config.clone(), "https://integrate.api.nvidia.com/v1"),
            "qwen/qwen3-5-122b-a10b"
        );
        assert_eq!(
            wire_model_for_base_url("z-ai/glm5.1", config.clone(), "https://integrate.api.nvidia.com/v1"),
            "z-ai/glm5.1"
        );
        assert_eq!(
            wire_model_for_base_url("nvidia/llama-3.3-nemotron-super-49b-v1.5", config.clone(), "https://integrate.api.nvidia.com/v1"),
            "nvidia/llama-3.3-nemotron-super-49b-v1.5"
        );

        // 8. Direct/single-provider endpoint strips other provider prefixes (like qwen/ or kimi/)
        assert_eq!(
            wire_model_for_base_url("qwen/qwen-max", config.clone(), "https://dashscope.aliyuncs.com/compatible-mode/v1"),
            "qwen-max"
        );
        assert_eq!(
            wire_model_for_base_url("kimi/kimi-k2.5", config.clone(), "https://dashscope.aliyuncs.com/compatible-mode/v1"),
            "kimi-k2.5"
        );

        // 9. Standard OpenAI endpoint strips openai/ prefix
        assert_eq!(
            wire_model_for_base_url("openai/gpt-4o", config, "https://api.openai.com/v1"),
            "gpt-4o"
        );
    }

    #[test]
    fn test_read_base_url_strips_anthropic_suffix() {
        ...
        // Case 1: Default base URL has /anthropic suffix
        assert_eq!(read_base_url(config), "https://api.deepseek.com");

        // Case 2: Environment variable is set with /anthropic suffix
        std::env::set_var("TEST_OPENAI_BASE_URL", "https://api.deepseek.com/anthropic/");
        assert_eq!(read_base_url(config), "https://api.deepseek.com");

        // Case 3: Environment variable is set with uppercase /ANTHROPIC suffix
        std::env::set_var("TEST_OPENAI_BASE_URL", "https://api.deepseek.com/ANTHROPIC");
        assert_eq!(read_base_url(config), "https://api.deepseek.com");

        // Case 4: Environment variable without /anthropic suffix
        std::env::set_var("TEST_OPENAI_BASE_URL", "https://api.deepseek.com/v1");
        assert_eq!(read_base_url(config), "https://api.deepseek.com/v1");

        // Clean up
        std::env::remove_var("TEST_OPENAI_BASE_URL");
    }
}
